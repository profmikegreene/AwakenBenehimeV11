...
* -----------------------------------------------------------------*/
	...
	$('.horz-menu').each(function() {
		horizontal_dropdown($(this));
	});

	function horizontal_dropdown($menu) {
		...
		var adjust_menu = function() {
			...
			if ($window_width < 822) {
				...
				$menu_list.show();
				$menu_list.find("li").removeClass("is-hover");
				$menu_list.find("li a").unbind('click');
				$menu_list.find("li").unbind('mouseenter mouseleave').bind('mouseenter mouseleave', function() {
					// must be attached to li so that mouseleave is not triggered when hover over submenu
					$(this).toggleClass('is-hover');
				});
				$menu_list.find("li a.is-parent").unbind('click').bind('click', function(e) {
					// must be attached to anchor element to prevent bubbling
					e.preventDefault();
				});
			}
...
// $('.vert-menu--sidebar').find(".menu-item").has('ul').prepend('<span class="icon icon-dropdown-arrow">&#9660;</span>');
